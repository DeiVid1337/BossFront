...
/**
 * Seller Stock Management Stock Edit View
 * Página para admin/manager editarem o estoque de um produto específico de um vendedor
...
// Computed
const currentStock = computed(() => storeProduct.value?.stock_quantity || 0)
const minStockLevel = computed(() => storeProduct.value?.min_stock_level || 0)

const newStock = computed(() => {
  if (!storeProduct.value) return 0
  
  switch (adjustmentType.value) {
    case 'set':
      return adjustmentAmount.value
    case 'add':
      return currentStock.value + adjustmentAmount.value
    case 'subtract':
      return currentStock.value - adjustmentAmount.value
    default:
...
const isLowStock = computed(() => {
  return newStock.value <= minStockLevel.value
})
...
async function handleSubmit() {
  ...
  if (adjustmentAmount.value < 0) {
    errors.adjustment_amount = ['A quantidade não pode ser negativa']
  }

  if (adjustmentType.value === 'set' && adjustmentAmount.value < 0) {
    errors.adjustment_amount = ['O estoque não pode ser negativo']
  }

  if (adjustmentType.value === 'subtract' && adjustmentAmount.value > currentStock.value) {
  ...
  try {
    ...
    stockSync.emitStockUpdated({ source: 'seller-stock-management-stock-edit' })
    router.push({
      path: `/seller-stock-management/${sellerId}/stores/${storeId.value}/products`,
    })
  } catch (err) {
...
function cancel() {
  router.push(`/seller-stock-management/${sellerId}/stores/${storeId.value}/products`)
}

function getProductLabel(): string {
  if (storeProduct.value?.product) {
    const product = storeProduct.value.product
    return `${product.brand} ${product.name}${product.flavor ? ` - ${product.flavor}` : ''}`
  }
  return `Produto #${storeProduct.value?.product_id || ''}`
}

onMounted(() => {
  syncUrlToStore(id => ({
    path: `/seller-stock-management/${sellerId}/stores/${id}/products/${storeProductId}/edit-stock`,
  }))
  loadStoreProduct()
})
...
watch(effectiveStoreId, (newId, oldId) => {
  ...
  if (routeStoreId.value !== newId) {
    router.replace({
      path: `/seller-stock-management/${sellerId}/stores/${newId}/products/${storeProductId}/edit-stock`,
    })
  }
  loadStoreProduct()
})
...
<template>
  <div class="seller-stock-management-stock-edit-page">
    ...
    <div class="page-container">
      ...
      <div v-else-if="error" class="error-state">
        <div class="error-card">
          ...
          <button @click="cancel" class="btn-back-to-list">
            Voltar para Lista
          </button>
        </div>
      </div>
      ...
      <div v-else-if="storeProduct" class="form-card">
        ...
        <div class="product-info-section">
          ...
          <div class="product-info">
            <div class="product-name">{{ getProductLabel() }}</div>
            <div class="product-details">
              ...
              <div class="detail-item">
                ...
                <span class="detail-value stock-current">{{ currentStock }} unidades</span>
              </div>
              <div class="detail-item">
                <span class="detail-label">Estoque Mínimo:</span>
                <span class="detail-value">{{ minStockLevel }} unidades</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="adjustment-section">
          ...
          <div class="form-group">
            ...
            <input
              ...
              step="1"
              class="form-input"
              :class="{ 'input-error': fieldErrors.adjustment_amount }"
              placeholder="0"
            />
          ...
          <div class="preview-section">
            <div class="preview-card">
              ...
              <div
                :class="[
                  'preview-value',
                  isLowStock ? 'preview-value-low' : 'preview-value-ok'
                ]"
              >
                {{ newStock }} unidades
              </div>
              <div v-if="isLowStock" class="preview-warning">
                ⚠️ Estoque ficará abaixo do mínimo ({{ minStockLevel }} unidades)
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="form-actions">
          ...
          <button
            @click="handleSubmit"
            :disabled="submitting || adjustmentAmount < 0 || (adjustmentType === 'subtract' && adjustmentAmount > currentStock)"
            class="btn-submit"
          >
...
.seller-stock-management-stock-edit-page {
  min-height: 100vh;
